...
namespace GAM
{
    public partial class FrmChargeMan : Form
    {
        ...
        private void DisplayData(SQLiteConnection con)
        {
            ...
            string sql = "select id,name,phone,sex from charge_man where delflag=0";
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            con.Execute(" insert into charge_man(name,sex,birth,phone,addr,remark) values (@name,@sex,@birth,@phone,@addr,@remark)"
            , new
            {
                name = name,
                sex = sex,
        ...
        private void btnModify_Click(object sender, EventArgs e)
        {
            ...
            con.Execute(" update charge_man set name=@name,sex=@sex,birth=@birth,phone=@phone,addr=@addr,remark=@remark where id=@userid"
            , new
            {
                name = name,
                sex = sex,
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            string userid = txtID.Text.Trim();
            con.Execute(" update charge_man set delflag=@delflag where id=@userid"
            , new
            {
                delflag = true,
                userid = userid
        ...
        private void dataGridView1_RowHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            var cm = con.Query<ChargeMan>("select * from charge_man where id=@userid", new
            {
                userid = userid
            }).SingleOrDefault();
        ...
        private void FrmChargeMan_Load(object sender, EventArgs e)
        {
            initInput();
            SQLiteConnectionStringBuilder sb = new SQLiteConnectionStringBuilder();
            sb.DataSource = databasename;
            ...
            if (openFileDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                try
                {
                    using (ExcelHelper excelHelper = new ExcelHelper(fName))
                    {
                        ...
                        SQLiteConnection con = new SQLiteConnection(sb.ToString());
                        con.Open();
                        con.Execute(" insert into charge_man(name,sex,birth,phone,addr,remark) values (@name,@sex,@birth,@phone,@addr,@remark)", cms);
                        DisplayData(con);
                        con.Close();
                        initInput();
